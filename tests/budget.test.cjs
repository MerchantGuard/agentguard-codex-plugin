...
test('warm hook budget defaults to 250 ms and leaves exit margin below the host ceiling', () => {
  for (const value of [undefined, null, '40', 0, -1, NaN, Infinity, 1.5]) assert.equal(normalizeBudget(value), 250);
  assert.equal(normalizeBudget(400), 400); assert.equal(normalizeBudget(40), 40);
...
test('hookBudgetMs follows policy changes while malformed policy uses the safe default deadline', t => {
  ...
  fs.writeFileSync(file,JSON.stringify({version:1,mode:'shadow',hookBudgetMs:90}));assert.equal(warmBudget(data),90);
