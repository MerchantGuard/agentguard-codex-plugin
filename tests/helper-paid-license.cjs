...
function seedPaidLicense(home, key = LICENSE_KEY, data) {
  ...
  if (data) {
    ...
    fs.writeFileSync(path.join(data, 'org-policy-status.json'), JSON.stringify({
      license_fingerprint: fingerprint, status: 'none', reason: null,
      org_policy_sha256: null, updated_at: new Date().toISOString(),
    }) + '\n', {mode: 0o600});
  }
  return key;
}
